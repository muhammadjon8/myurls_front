...
async function addNewLink() {
  ...
  try {
    await axios.post("https://95.130.227.35/api/user-links", {
      user_id: props.id,
...
async function updateLink(link) {
  ...
  // console.log(link)
  try {
    await axios.patch(`https://95.130.227.35/api/user-links/${link.id}`, {
...
async function deleteLink(link) {
  try {
    await axios.delete(`https://95.130.227.35/api/user-links/${link.id}`);
